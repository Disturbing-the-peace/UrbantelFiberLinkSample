...
import { verifyToken, checkSuperadmin, checkAdmin } from '../../shared/middleware/auth';
...
router.post('/', verifyToken, checkAdmin, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error in POST /api/agents:', error);
...
router.get('/by-referral/:referralCode', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error in GET /api/agents/by-referral/:referralCode:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

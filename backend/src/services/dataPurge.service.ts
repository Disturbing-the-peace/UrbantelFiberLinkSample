...
export class DataPurgeService {
  ...
  startScheduler(): void {
    if (this.cronJob) {
      console.log('Data purge scheduler is already running');
      return;
    }

    // Schedule to run daily at 2:00 AM
    this.cronJob = cron.schedule('0 2 * * *', async () => {
      console.log('Running scheduled data purge job...');
      try {
        const results = await this.executePurge();
        console.log(`Data purge completed. Purged ${results.length} records.`);
      } catch (error) {
        console.error('Error during scheduled data purge:', error);
      }
    });

    console.log('Data purge scheduler started (runs daily at 2:00 AM)');
  }
  ...
  stopScheduler(): void {
    if (this.cronJob) {
      this.cronJob.stop();
      this.cronJob = null;
      console.log('Data purge scheduler stopped');
    }
  }
  ...
  async executePurge(): Promise<PurgeResult[]> {
    ...
    if (!subscribers || subscribers.length === 0) {
      console.log('No subscribers found for data purge');
      return [];
    }
    ...
    for (const subscriber of subscribers) {
      try {
        ...
        console.error(`Failed to purge data for subscriber ${subscriber.id}:`, error);
      }
    }
  ...
  private async purgeSubscriberData(subscriber: any): Promise<PurgeResult> {
    ...
    for (const fileField of fileFields) {
      if (fileField.url) {
        try {
          ...
          console.error(`Error deleting file for ${fileField.field}:`, error);
        }
      }
    }
    ...
    if (auditError) {
      console.error(`Failed to create audit log for subscriber ${subscriber.id}:`, auditError.message);
    }
  ...
  async manualPurge(): Promise<PurgeResult[]> {
    console.log('Executing manual data purge...');

...
class SMSGatewayService {
  ...
  async sendSMS(phone: string, message: string): Promise<NotificationResult> {
    try {
      ...
      logger.log(`[SMS Gateway] Sending SMS to ${phone}: ${message}`);
      
      // Simulate successful send in development
      if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
        logger.log('[SMS Gateway] SMS sent successfully (simulated)');
        return {
          success: true,
          channel: 'sms',
        };
      ...
      logger.error('[SMS Gateway] Error sending SMS:', error);
      return {
        success: false,
        channel: 'sms',
...
export class NotificationService {
  ...
  async sendStatusChangeNotification(
    ...
    if (!notifiableStatuses.includes(status)) {
      logger.log(`[Notification Service] Status '${status}' does not trigger notifications`);
      return;
    }
    ...
    logger.log(`[Notification Service] Sending ${status} notification to customer: ${customerName}`);
    ...
    customerResults.forEach(result => {
      if (result.success) {
        logger.log(`[Notification Service] Customer notification sent via ${result.channel}`);
      } else {
        logger.error(`[Notification Service] Failed to send customer notification via ${result.channel}: ${result.error}`);
      }
    });
